...
if(isTRUE(random)){
    seq.ID.string <- data.frame(seq.ID=unique(tmp$seq.ID), number=sample(1:length(unique(tmp$seq.ID))))
  } else if(!isTRUE(random)){
    seq.ID.string <- data.frame(seq.ID=unique(tmp$seq.ID), number=(1:length(unique(tmp$seq.ID))))
}

tmp <- left_join(tmp,seq.ID.string, by="seq.ID")

plot <- ggplot() + theme_bw() +
  facet_wrap(~sample_name) +
  geom_bar(data=tmp, aes(number, abundance, fill=seq.ID), color="black", stat="identity", show.legend=FALSE) +
